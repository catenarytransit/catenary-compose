...
@Composable
fun AddShapes(layerSettings: AllLayerSettings, railInFrame: Boolean = false) {
    val busShapesSource = rememberVectorSource(uri = SHAPES_SOURCES.getValue("busshapes"))
    val otherShapesSource = rememberVectorSource(uri = SHAPES_SOURCES.getValue("othershapes"))
    val intercityRailSource =
            rememberVectorSource(uri = SHAPES_SOURCES.getValue("intercityrailshapes"))
    val localCityRailSource =
            rememberVectorSource(uri = SHAPES_SOURCES.getValue("localcityrailshapes"))
    ...
    var busTextSize: Expression<TextUnitValue> =
            interpolate(
                    type = linear(),
                    input = zoom(),
    ...
    SymbolLayer(
            ...
            textFont = const(listOf("Arimo-Regular")),
    ...
    LineLayer(
            ...
            // filter: ! (chateau=='schweiz' && stop_to_stop_generated==true)  && (route_type==6 ||
            // route_type==7)
            filter =
                    all(
                            all(
    ...
    val line_opacity_intercity: Expression<NumberValue<Number>> =
            switch(
                    condition(
    ...
    SymbolLayer(
            ...
            textField =
                    get("route_label")
                            .cast<StringValue>(), // your JS toggles debug; you can replicate if
            // needed
            textFont =
                    step(
                            input = zoom(),
                            const(listOf("Arimo-SemiBold")),
                            8.0 to const(listOf("Arimo-Bold"))
                    ),
            textSize =
                    interpolate(
                            type = linear(),
    ...
    SymbolLayer(
            ...
            placement = const(SymbolPlacement.Line),
            textField = coalesce(get("route_label").cast(), const("")),
            textFont = const(listOf("Arimo-Bold")),
            textSize =
                    interpolate(
                            type = linear(),
                            input = zoom(),
    ...
    val tram_filter: Expression<BooleanValue> =
            all(
                    any(
    ...
    SymbolLayer(
            ...
            textFont =
                    step(
                            ...
                            12.0 to const(listOf("Arimo-Medium"))
                    ),
            textSize =
                    interpolate(
                            type = linear(),
